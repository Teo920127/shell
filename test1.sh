#!/bin/bash
# install Nginx 1.6.2 + mysql5.5.x + PHP-FPM 5.4.x + Zabbix 2.4.4 + phpMyAdmin-4.1.12 automatically.
# Test on CentOS 6.5
##############################################
# 变量
##############################################
...
# Nginx 
#########################################
info_echo "Nginx setup file update"
...
info_echo "zabbix add setup file"
...
info_echo "mkdir wep dir"
mkdir -p /data/web/zabbix
 
info_echo "Downloading..."
if [ ! -f /tmp/zabbix.tar.gz ]; then
    mv $(cd `dirname "$0"`;pwd)/zabbix-2.4.5.tar /tmp/zabbix.tar.gz
fi
...
mv /tmp/${ZABBIX_DIR}/frontends/php/* /var/www/zabbix/
chown -R deploy.deploy /var/www/zabbix/
